    ...
    process
    {
        try
        {
            if ($Name -ne $null)
            {
                ...
                $RestoreConfigTableFromBin=$false
                if ($BackupFileObj.PSIsContainer)
                {
                    $Format="Directory"
                    $ConfigTableFullPath=Join-Path -Path $BackupFileObj.FullName -ChildPath $($BackupFileObj.Name+".bin")
                    if (Test-Path -Path $ConfigTableFullPath)
                    {
                        $RestoreConfigTableFromBin=$true
                    }
                }
                else
                {
                    ...
                    if ($PSBoundParameters["Format"] -eq $null)
                    {
                        ...
                        {
                            $Format=$FormatTmp 
                            $ConfigTableFullPath=$BackupFileObj.FullName -replace "\$($BackupFileObj.Extension)$",".bin"
                            if (Test-Path -Path $ConfigTableFullPath)
                            {
                                $RestoreConfigTableFromBin=$true    
                            }
                ...
                Write-Verbose "$RecoveryName :start restore from file $RestoreBackupFile Please wait.." -Verbose
                Write-Verbose "$PgAppPath $PgRestoreArgs"
                ...
                if ($Result.ExitCode -eq 0)
                {
                    if ($RestoreConfigTableFromBin)
                    {
                        Write-Verbose "$RecoveryName :start restore public.config from file $ConfigTableFullPath" -Verbose
                        $RestoreConfigTableQuery="\COPY public.config FROM "+"'"+$($ConfigTableFullPath -replace "\\","\\")+"'"+"  WITH BINARY;"
                        Invoke-PgQuery -Name $RecoveryName -Query $RestoreConfigTableQuery -ReturnAs Raw -ErrorAction Stop | Out-Null 
                    }
                    Write-Verbose "$RecoveryName Success restored from backup $BackupFile" -Verbose
                    if ($Name -ne $RecoveryName)
                    {
                        Write-Verbose "Rename-PgDatabase -Name $RecoveryName -NewName $Name" -Verbose
...
function Backup-PgDatabase
{
    ...
    param(
        ...
        [string]$Name,
        [string]$Destination,
        [ValidateSet("Custom","Directory","CompressDirectory","Tar","Plain","CompressPlain")]
    ...
    process
    {
        try
        {
            if ($Name -ne $null)
            {
                ...
                $BackupFileNameBinary=$BackupFileName+".bin"
                ...
                $LargeBinaryStringInTableConfig=$False
                try
                {
                    $MaxBinarySizeInDB=(((Invoke-PgQuery -Query 'select octet_length(binarydata) as "octet_length" from config order by octet_length desc limit 1' -Name $Name -ErrorAction Stop).octet_length -as [int64]))
                    if ($MaxBinarySizeInDB -gt $MaxBinaryStringSize)
                    {
                        $LargeBinaryStringInTableConfig=$true
                ...
                if ($LargeBinaryStringInTableConfig)
                {
                    Write-Verbose "$($Name): One of the public.config table entries is large. Two backup files will be created. Read more https://infostart.ru/1c/articles/956734/" -Verbose
                    $DestinationBinaryPath=Join-Path -Path $Destination -ChildPath $BackupFileNameBinary
                    $CopyConfigQuery="\COPY public.config TO "+"'"+$($DestinationBinaryPath -replace "\\","\\")+"'"+" WITH BINARY;"
                    Write-Verbose "$($Name+':') Starting backup public.config to file $DestinationBinaryPath Please wait.." -Verbose
                    Invoke-PgQuery -Query $CopyConfigQuery -Name $Name -ReturnAs Raw -ErrorAction Stop | Out-Null
                    $PgArg+="--exclude-table-data=public.config"    
                }
                ...
                Write-Verbose "$($Name+':') Starting backup to file $DestinationFullPath Please wait.." -Verbose
                ...
                if ($Result.exitcode -eq 0)
                {
                    if ($($Format -eq "Directory" -or $Format -eq "CompressDirectory") -and $LargeBinaryStringInTableConfig)
                    {
                    ...
                    if ($PSBoundParameters["Format"] -eq "CompressDirectory")
                    {
                        ...
                        Write-Verbose "Backup success. Try Compress7zip -Path $CompressFiles -NoCompression -DelAfterCompress"
                    ...
                    if ($PSBoundParameters["Format"] -eq "CompressPlain")
                    {
                        $CompressFiles=@()
                        $CompressFiles+=$($OutRes.BackupFile)
                        if ($LargeBinaryStringInTableConfig)
                        {
                            $CompressFiles+=$DestinationBinaryPath
                        }
                        if (!([string]::IsNullOrEmpty($($OutRes.LogFile))))
                        {
                            $CompressFiles+=$($OutRes.LogFile)
                        }
                        Write-Verbose "Backup success. Try Compress7zip -Path $CompressFiles -DelAfterCompress"
